...
void main() {
    ...
    // ── DNA Feedback Loop ──
    {
        ...
        float hue = u_time * u_param9 * 0.05 + u_energy * 0.3 + u_beat_phase * 0.1;
        float sat = 0.7 + u_stereo_width * 0.3;
        float val = length(p) * 0.3 + u_energy * 0.4;
        col += hsv2rgb(vec3(hue, sat, val)) * 0.6;
    }

    // ── FFT Bänder (oben) – direkt aus Textur sampeln ──
    {
        float bar_y = 1.0 - 0.15;
        if (uv.y > bar_y) {
            float local_y  = (uv.y - bar_y) / 0.15;
            float left     = texture(u_fft_left,  vec2(uv.x, 0.5)).r;
            float right    = texture(u_fft_right, vec2(uv.x, 0.5)).r;
    ...
    // ── Stereo Bild (rechts) – direkt aus Textur sampeln ──
    {
        float bar_x = 1.0 - 0.06;
        if (uv.x > bar_x) {
            float local_x = (uv.x - bar_x) / 0.06;
            float mid     = texture(u_fft_mid,  vec2(uv.y, 0.5)).r;
            float side    = texture(u_fft_side, vec2(uv.y, 0.5)).r;
            float is_mid  = step(1.0 - local_x, mid);
            float is_side = step(1.0 - local_x, side * u_stereo_width);
            vec3 sc = vec3(0.0);
            sc += vec3(0.9, 0.9, 0.9) * is_mid  * 0.7;
            sc += vec3(0.8, 0.3, 1.0) * is_side * 0.9;
            col = mix(col, sc, max(is_mid, is_side));
        }
    }
    ...
    // ── Beat Puls ──
    {
        ...
        col += mix(vec3(0.2, 0.4, 1.0), vec3(1.0, 0.2, 0.2), bn)
             * ring * (1.0 + u_beat_onset * 2.0);
    }
