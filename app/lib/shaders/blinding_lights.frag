...
float horizon_line(vec2 uv, float y, float softness) {
  return smoothstep(y - softness, y + softness, uv.y);
}

float retro_grid(vec2 uv, float freq, float y, float time) {
  vec2 p = uv;
  p.y -= y;
...
float skyline(vec2 uv, float y, float time, float seed) {
  ...
  float neonWindow = w * step(0.4, hash(floor(wp) + seed));
  return building * 0.6 + neonWindow * 0.4;
}

void main() {
  float d_style        = hash_seed(u_seed,  1.0);
  float d_sun_size     = 0.3 + hash_seed(u_seed,  2.0) * 0.7;
  float d_sun_y        = -0.4 + hash_seed(u_seed,  3.0) * 0.8;
  float d_color_speed  = 0.05 + hash_seed(u_seed,  4.0) * 0.6;
  float d_color_offset = hash_seed(u_seed,  5.0);
  float d_bass_react   = 0.4 + hash_seed(u_seed,  6.0) * 1.0;
  float d_mid_react    = 0.4 + hash_seed(u_seed,  7.0) * 1.0;
  float d_high_react   = 0.3 + hash_seed(u_seed,  8.0) * 1.0;
  float d_rotation     = (hash_seed(u_seed,  9.0) - 0.5) * 0.5;
  float d_warp_x       = (hash_seed(u_seed, 10.0) - 0.5) * 1.4;
  float d_warp_y       = (hash_seed(u_seed, 11.0) - 0.5) * 1.4;
  float d_pulse_str    = 0.2 + hash_seed(u_seed, 12.0) * 1.0;
  float d_stereo_str   = 0.05 + hash_seed(u_seed, 13.0) * 0.4;
  float d_fft_react    = 0.2 + hash_seed(u_seed, 14.0) * 1.0;
  float d_laser_count  = floor(3.0 + hash_seed(u_seed, 15.0) * 9.0);
  float d_grid_size    = 3.0 + hash_seed(u_seed, 16.0) * 8.0;
  float d_grid_persp   = 0.3 + hash_seed(u_seed, 17.0) * 0.7;
  float d_skyline_h    = 0.1 + hash_seed(u_seed, 18.0) * 0.4;
  float d_detail_amp   = 0.5 + hash_seed(u_seed, 19.0) * 1.5;
  float d_cloud_amp    = hash_seed(u_seed, 20.0);
  ...
  if (d_style < 0.33) {
    ...
    vec2 p = uv - vec2(0.0, sunY);
    float r = length(p);
    float glow = exp(-r * (4.0 / (d_sun_size * pulse))) * 0.8;
    float core = exp(-r * (12.0 / (d_sun_size * pulse))) * 1.2;
    float stripeAngle = atan(p.y, p.x);
    float stripeMask = smoothstep(0.15, 0.0, abs(fract(stripeAngle / 6.28318 * 8.0) - 0.5));
    float stripes = stripeMask * smoothstep(d_sun_size * 0.6, 0.0, r) * 0.5;
    vec3 colTop = pal(fract(d_color_offset + u_time * d_color_speed));
    vec3 colBot = pal(fract(d_color_offset + u_time * d_color_speed + 0.4));
    vec3 sunCol = mix(colBot, colTop, smoothstep(0.0, d_sun_size, p.y + d_sun_size * 0.5));
    col += sunCol * (glow + stripes + core);
    float horY = sunY - d_sun_size * 0.5;
    float h = smoothstep(0.02, 0.0, abs(uv.y - horY));
    float stripes2 = 0.0;
    for (int i = 0; i < 8; i++) {
      float fi = float(i);
      float sy = horY + (fi - 3.5) * 0.02;
      float move = sin(u_time * 0.5 + fi * 1.3) * 0.01;
      float line = smoothstep(0.004, 0.0, abs(uv.y - (sy + move)));
      stripes2 += line * (1.0 - fi * 0.1);
    }
    float neonLine = max(h, stripes2 * 0.7);
    col += u_pal_highlight * neonLine * (1.2 + u_bass * d_bass_react);
    float grid = retro_grid(uv, d_grid_size, horY, u_time);
    ...
    float reflection = retro_grid(uv, d_grid_size, d_skyline_h - 0.05, u_time) * 0.4;
  ...
  for (int i = 0; i < 16; i++) {
    ...
    float hVal = mix(mix(u_bass, u_mid, clamp(pos * 2.0, 0.0, 1.0)), u_high, clamp(pos * 2.0 - 1.0, 0.0, 1.0));
    float barH = hVal * 1.2;
    float top = 0.9;
