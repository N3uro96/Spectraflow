...
void BPMDetector::process(const AudioData& audio, size_t num_samples, float& bpm_out,
                           bool& beat_onset_out, float& beat_phase_out)
{
    // Exakte Berechnung der Zeit für diesen Frame
    frame_time_ = static_cast<float>(num_samples > 0 ? num_samples : 2048) / sample_rate_;
...
void BPMDetector::detect_onset(float energy)
{
    ...
    if (energy > avg * threshold_ && energy > 0.05f && time_since_last > min_beat_gap_) {
...
void BPMDetector::update_bpm()
{
    ...
    for (size_t i = 1; i < count; i++) {
        float interval = beat_times_[i % MAX_BEATS] - beat_times_[(i - 1) % MAX_BEATS];
